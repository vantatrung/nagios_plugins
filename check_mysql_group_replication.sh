...
crit="OFFLINE"		# what is the answer of MySQL Slave_SQL_Running for a Critical status?
ok="ONLINE"		# what is the answer of MySQL Slave_SQL_Running for an OK status?
...
ConnectionResult=`mysql -h ${host} -P ${port} -u ${user} --password=${password} -e "select * from performance_schema.replication_group_members where MEMBER_HOST='${host}'\G;" 2>&1`
if [ -z "`echo "${ConnectionResult}" |grep MEMBER_ID`" ]; then
	echo -e "CRITICAL: Unable to connect to server ${host}:${port} with username '${user}' and given password"
	exit ${STATE_CRITICAL}
fi
check=`echo "${ConnectionResult}" |grep MEMBER_STATE |awk '{print $2}'`
...
#########################################################################
if [ ${check} = "NULL" ]; then
echo "CRITICAL: MEMBER_STATE is answering NULL"; exit ${STATE_CRITICAL};
fi

if [ ${check} != ${ok} ]; then
echo "CRITICAL: ${host}:${port} MEMBER_STATE: ${check}"; exit ${STATE_CRITICAL};
fi
...
if [ ${check} = ${ok} ]; then
echo "OK: ${host}:${port} MEMBER_STATE: ${check}"; exit ${STATE_OK};
fi
